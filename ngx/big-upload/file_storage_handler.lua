...
-- Naive file storage implementation for each valid chunk status code "202 Accepted" is returned.
...
local function on_body_end(self, ctx)
  ...
  -- Ralph: for each chunk we return 202 ACCEPTED, I think it's more appropriate
  return {202, string.format("0-%d/%d", ctx.range_to, ctx.range_total) }
end
